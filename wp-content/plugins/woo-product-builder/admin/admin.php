	...
	public function admin_enqueue_script() {
		...
		if ( ( get_post_type() == 'woo_product_builder' && $screen->id == 'woo_product_builder' ) || $page == 'woocommerce-product-builder-setting' ) {
			...
			$arg_scripts = array(
				...
				'message_notice_4'         => esc_html__( 'You only add maximum 3 steps. Please upgrade Premium version.', 'woo-product-builder' ),
							...
							<div class="vi-ui tab <?php echo $k ? '' : 'active' ?>" data-tab="<?php echo esc_attr( $k ) ?>">
								...
								if ( is_array( $list_content ) && count( $list_content ) ) {
									foreach ( $list_content as $item ) {
										...
										?>
										<div class="woopb-item woopb-item-<?php echo $check_product ? 'product' : 'category' ?> <?php echo has_post_thumbnail( $item_data['id'] ) && $check_product ? 'woopb-img' : '' ?>" data-id="<?php echo esc_attr( $item_data['id'] ) ?>">
											...
											<input type="hidden" name="woopb-param[list_content][<?php echo esc_attr( $k ) ?>][]" value="<?php echo $check_product ? esc_attr( $item_data['id'] ) : 'cate_' . esc_attr( $item_data['id'] ) ?>">
										</div>
									<?php }
	...
	public function products_per_page_html() { ?>
		<table class="form-table vi-ui form">
			<tr valign="top">
				...
				<td>
					<input type="number" id="<?php echo self::set_field( 'product_per_page' ) ?>" name="<?php echo self::set_field( 'product_per_page' ) ?>" value="<?php echo self::get_field( 'product_per_page', 10 ) ?>" min="1" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
	...
	public function general_setting_html() { ?>
		<table class="form-table vi-ui form">
			<tr valign="top">
				...
				<td>
					<input type="text" name="<?php echo self::set_field( 'text_prefix' ); ?>" id="<?php echo self::set_field( 'text_prefix' ); ?>" value="<?php echo self::get_field( 'text_prefix', 'Step {step_number}' ); ?>">
			...
			<tr valign="top">
				...
				<td>
					<div class="vi-ui toggle checkbox checked">
						<input type="checkbox" name="<?php echo self::set_field( 'enable_quantity' ); ?>" id="<?php echo self::set_field( 'enable_quantity' ); ?>" <?php checked( self::get_field( 'enable_quantity' ), 1 ); ?> value="1">
			...
			<tr valign="top">
				...
				<td>
					<div class="vi-ui toggle checkbox checked">
						<input type="checkbox" name="<?php echo self::set_field( 'enable_preview' ); ?>" id="<?php echo self::set_field( 'enable_preview' ); ?>" <?php checked( self::get_field( 'enable_preview' ), 1 ); ?> value="1">
